...
const RegradeLensEntry = ({lenObj, divine}) => {
  ...
  return (
    <div>
      <label>
        {lenObj.currencyTypeName}: {lenObj.receive.value.toFixed(1)}<img src={chaos} className="max-w-5 max-h-5 inline"></img> OR {conversion()} <img src={div} className="max-w-5 max-h-5 inline"></img>
      </label>
    </div>
  )
}
